...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="min-h-[70vh] md:min-h-[40vh] lg:min-h-[70vh] justify-center flex flex-col">
      ...
      <FadeIn delay={0.1}>
        <p className="mt-8 text-xl font-light tracking-wide text-center md:mt-16 md:text-3xl text-neutral-300">
          Feedback from clients on great experiences
        </p>
      </FadeIn>
      <FadeIn delay={0.2}>
        <div className="flex flex-col items-center justify-center gap-8 mt-8">
          <div className="w-[90vw] overflow-x-hidden mt-4 md:mt-8">
            <div id="testimonials-container" className="flex flex-row transition-transform duration-700 ease-out">
              {testimoinals.map((t, i) => testimonial(t.description, t.linkTitle, t.link, i))}
            </div>
          </div>
...
const testimonial = (description: string, linkTitle: string, link: string, index: number) => {
  return (
